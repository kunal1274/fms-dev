...
const ConfirmSaleOrder = ({ handleAddSaleOrder, invoice }) => {
  ...
  const handleInvoice = () => {
    if (selectedSales.length === 1) {
      ...
      toast.warn(
        "⚠️ Please select exactly one sale order to generate an invoice.",
        {
          position: "top-right",
          autoClose: 3000,
  ...
  const fetchSales = useCallback(async () => {
    ...
    try {
      ...
      // Filter only those sales with status "Confirmed"
      const confirmedSales = response.data.data.filter(
        (sale) => sale.status === "Confirmed"
      );
      setSales(confirmedSales);
      setFilteredSales(confirmedSales);
      console.log(confirmedSales);
    } catch (error) {
  ...
  const applyFilters = useCallback(() => {
    ...
    // Additional filter: Only show confirmed sales
    filtered = filtered.filter((sale) => sale.status === "Confirmed");
  ...
  const generatePDF = useCallback(() => {
    if (!selectedSales || selectedSales.length === 0) {
      ...
      toast.warn("No sales selected to generate PDF!", {
        position: "top-right",
        autoClose: 3000,
        hideProgressBar: false,
        closeOnClick: true,
...
export default ConfirmSaleOrder;
