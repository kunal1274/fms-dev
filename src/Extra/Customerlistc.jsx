...
export default function CustomerList({ handleAddCustomer, onView }) {
  useEffect(() => {
    const alreadyReloaded = sessionStorage.getItem("customerListReloaded");
    if (!alreadyReloaded) {
      sessionStorage.setItem("customerListReloaded", "true");
  ...
  const openDetail = (id) => setViewingCustomerId(id);
  ...
  const handleDeleteSelected = async () => {
    if (!selectedCustomers.length) return toast.info("No customers selected");
  ...
  const exportToExcel = () => {
    if (!customerList.length) return toast.info("No data to export.");
