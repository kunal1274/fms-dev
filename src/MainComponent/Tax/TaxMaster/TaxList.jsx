...
import "./c.css";
import TaxListViewPage from "./TaxViewPage";

export default function TaxListList({ handleAddTaxList }) {
  const baseUrl = "https://fms-qkmw.onrender.com/fms/api/v0/TaxLists";
  ...
  const tabNames = [
    ...
    "Active TaxLists",
    "On‑Hold TaxLists",
    "Outstanding TaxLists",
  ];

  const [activeTab, setActiveTab] = useState(tabNames[0]);
  const [TaxLists, setTaxLists] = useState([]);
  const [filteredTaxLists, setFilteredTaxLists] = useState([]);
  ...
  // Fetch TaxLists
  const fetchTaxLists = useCallback(
    async (from = startDate, to = endDate) => {
      ...
      try {
        ...
        console.error(err);
        setError("Unable to load TaxList data.");
      } finally {
        setLoading(false);
      }
    },
  ...
  const fetchMetrics = useCallback(async () => {
    ...
    try {
      ...
      setSummary((prev) => ({
        ...prev,
        count: m.totalTaxLists ?? prev.count,
        creditLimit: m.creditLimit ?? prev.creditLimit,
        paid: m.paidTaxLists ?? prev.paid,
        active: m.activeTaxLists ?? prev.active,
        onHold: m.onHoldTaxLists ?? prev.onHold,
      }));
    } catch (err) {
      console.error(err);
    } finally {
  ...
  useEffect(() => {
    ...
    setFilteredTaxLists(list);
  }, [TaxLists, activeTab, statusFilter, searchTerm, sortOption]);
  ...
  const toggleSelectAll = (e) =>
    setSelectedIds(e.target.checked ? filteredTaxLists.map((w) => w._id) : []);
  ...
  const handleDeleteSelected = async () => {
    if (!selectedIds.length) return toast.info("No TaxLists selected");
    if (!window.confirm("Delete selected TaxLists?")) return;
  ...
  const exportToExcel = () => {
    if (!TaxLists.length) return toast.info("No data to export");
    const ws = XLSX.utils.json_to_sheet(TaxLists);
    const wb = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(wb, ws, "TaxLists");
    XLSX.writeFile(wb, "TaxLists.xlsx");
  };
  ...
  if (viewingId) {
    return <TaxListViewPage TaxListId={viewingId} goBack={goBack} />;
  }
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-between ">
                <div className="flex items-center space-x-2">
                  ...
                  <h3 className="text-xl font-semibold">TaxList</h3>
                </div>
                <div className="flex items-center gap-3 ">
                  <button
                    onClick={handleAddTaxList}
              ...
              <div className=" bg-white rounded-lg ">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
                  {[
                    ["Total TaxLists", summary.count],
                    ["Credit Limit", summary.creditLimit],
                    ["Paid TaxLists", summary.paid],
                    ["Active TaxLists", summary.active],
                    ["On-Hold TaxLists", summary.onHold],
                  ].map(([label, value]) => (
                    <div
                      key={label}
              ...
              <div className="table-scroll-container h-[400px] overflow-auto bg-white rounded-lg">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tr>
                      <th className="sticky top-0 z-10 px-4 py-2 bg-gray-50">
                        <input
                          ...
                          checked={
                            selectedIds.length === filteredTaxLists.length &&
                            filteredTaxLists.length > 0
                          }
                        />
                      </th>
                      {[
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredTaxLists.length ? (
                      filteredTaxLists.map((w, i) => (
                        <tr
                          key={w._id}
