...
const API_BASE = "https://fms-qkmw.onrender.com/fms/api/v0/Serialss";
const SITES_API = "https://fms-qkmw.onrender.com/fms/api/v0/sites";

export default function SerialsViewPage({ SerialsId: propId, goBack }) {
  // Determine ID from prop or URL
  const { id: paramId } = useParams();
  const SerialsId = propId || paramId;
  ...
  // Fetch Serials detail
  useEffect(() => {
    async function fetchDetail() {
      setLoading(true);
      try {
        const resp = await axios.get(`${API_BASE}/${SerialsId}`);
  ...
  const handleUpdate = async () => {
    if (!window.confirm("Are you sure you want to update this Serials?"))
      return;
    setLoading(true);
    const toastId = toast.loading("Updating...");
    try {
      const resp = await axios.put(`${API_BASE}/${SerialsId}`, formData);
  ...
  return (
    <div className="p-6 bg-white rounded shadow">
      ...
      <header className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-semibold">Serials Details</h2>
