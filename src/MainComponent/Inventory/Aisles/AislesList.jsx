...
const AislesList = ({ handleAddAisles, onView }) => {
  ...
  const [AislesSummary, setAislesSummary] = useState({
    count: 0,
    creditLimit: 0,
    paidAisless: 0,
    activeAisless: 0,
  ...
  const handleFilterChange = (e) => {
    ...
    let filtered = [...AislesList];

    if (value === "All") {
      setFilteredAisles(filtered);
    } else if (value === "yes") {
      setFilteredAisles(filtered.filter((aisle) => aisle.active === true));
    } else if (value === "no") {
      setFilteredAisles(filtered.filter((aisle) => aisle.active === false));
  ...
  const fetchAisles = useCallback(
    async (fromDate = startDate, toDate = endDate) => {
      ...
      try {
        ...
        +setFilteredAisles(list); // ← Add this line to update the visible Aisles immediately

        setAislesSummary({
          count: list.length,
          creditLimit: list.reduce((s, c) => s + (c.creditLimit || 0), 0),
          paidAisles: list.filter((c) => c.status === "Paid").length,
          activeAisles: list.filter((c) => c.active).length,
          onHoldAisles: list.filter((c) => c.onHold).length,
        });
      } catch (err) {
        console.error(err);
  ...
  useEffect(() => {
    ...
    setFilteredAisles(list);
  }, [aislesList, activeTab, statusFilter, searchTerm, sortOption]);
  ...
  const toggleSelectAll = (e) => {
    setSelectedAisles(e.target.checked ? filteredAisles.map((c) => c._id) : []);
  };
  ...
  const handleDeleteSelected = async () => {
    if (!selectedAisles.length) {
      toast.info("No Aisles selected to delete");
      return;
    }

    if (!window.confirm("Delete selected Aisles?")) return;

    try {
      const results = await Promise.allSettled(
        selectedAisles.map((id) => axios.delete(`${baseUrl}/${id}`))
      );
  ...
  const handleAislesClick = (AislesId) => {
    setViewingAislesId(AislesId);
  };
  ...
  const goBack = () => setViewingAislesId(null);
  ...
  if (viewingAislesId) {
    return (
      <div className="p-4">
        <AisleViewPage aisleId={viewingAislesId} goBack={goBack} />
      </div>
    );
  }
  return (
            ...
            <div className="space-y-6">
              ...
              <div className="flex justify-between ">
                ...
                <div className="flex items-center gap-3 ">
                  ...
                  <button
                    ...
                    disabled={!selectedAisles.length}
                    className="h-8 px-3 border border-green-500 bg-white text-sm rounded-md transition hover:bg-blue-500 hover:text-blue-700 hover:scale-[1.02]"
                  >
                    Delete
                  </button>
              ...
              <div className=" bg-white rounded-lg ">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
                  {[
                    ["Total Aisles", AislesSummary.count],
                    ["Credit Limit", AislesSummary.creditLimit],
                    ["Paid Aisles", AislesSummary.paidAisles],
                    ["Active Aisles", AislesSummary.activeAisles],
                    ["On‑Hold Aisles", AislesSummary.onHoldAisles],
                  ].map(([label, value]) => (
                    <div
                      key={label}
              ...
              <div className="table-scroll-container h-[400px] overflow-auto bg-white rounded-lg">
                <table className="min-w-full divide-y divide-gray-200">
                    ...
                    <tr>
                      <th className="sticky top-0 z-10 px-4 py-2 bg-gray-50">
                        <input
                          ...
                          checked={
                            selectedAisles.length === filteredAisles.length &&
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredAisles.length ? (
                      filteredAisles.map((c) => (
                        <tr
                          key={c.code}
                        ...
                        >
                          <td className="px-4 py-2">
                            <input
                              type="checkbox"
                              checked={selectedAisles.includes(c._id)}
