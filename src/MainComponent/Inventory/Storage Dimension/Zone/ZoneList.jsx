...
import ZoneViewPagee from "./zoneViewPagee";

export default function zoneList({ handleAddzone, onView }) {
  /** ---------- API ---------- */
    ...
    async (fromDate = startDate, toDate = endDate) => {
      ...
      try {
        ...
        setSummary((prev) => ({
          ...
          archivedzones: m?.archivedzones ?? prev.archivedzones,
        }));
      } catch (err) {
        console.error(err);
        // metrics optional
  ...
  const filteredzone = useMemo(() => {
    ...
    if (term) {
      list = list.filter((c) => {
        const hay = [getName(c), getCode(c), getType(c), getAddresss(c)]
          .join(" | ")
          .toLowerCase();
    ...
    return list;
  }, [zone, activeTab, statusFilter, searchTerm, sortOption]);
  ...
  const toggleSelectAll = (e) => {
    setSelectedIds(e.target.checked ? filteredzone.map((c) => getId(c)) : []);
  };
  ...
  const handleDeleteSelected = async () => {
    if (!selectedIds.length) {
      toast.info("No zone selected to delete");
      return;
    }
    if (
      !window.confirm(
        `Delete ${selectedIds.length} selected zone${
    ...
    try {
      ...
      if (succeeded) {
        ...
        setSelectedIds([]);
        await fetchzone(startDate, endDate);
  ...
  const exportToExcel = () => {
    const rows = filteredzone.length ? filteredzone : zone;
    if (!rows.length) {
  ...
  if (viewingzoneId) {
    return (
      <div className="p-4">
        <ZoneViewPagee zoneId={viewingzoneId} goBack={goBack} />
      </div>
    );
  }
          ...
          <div className="space-y-6">
            ...
            <div className=" bg-white rounded-lg ">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
                {[
                  ["Total zones", summary.count],
                  ["Active zones", summary.activezones],
                  ["Archived zones", summary.archivedzones],
                  [
                    "Inactive (calc)",
                    Math.max(summary.count - summary.activezones, 0),
                  ],
                ].map(([label, value]) => (
                  <div
                    key={label}
              ...
              <div className="flex flex-wrap items-center gap-3 md:gap-4">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <option value="name-asc">zone Name</option>
                    <option value="code-asc">zone Account in Ascending</option>
                  ...
                  <tr>
                    <th className="sticky top-0 z-10 px-4 py-2 bg-gray-50">
                      <input
                        ...
                        checked={
                          selectedIds.length === filteredzone.length &&
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <button
                            ...
                            onClick={() => handlezoneClick(getId(c))}
                          >
                            {getCode(c)}
                          </button>
                        </td>
