...
const mergedUrl = `https://fms-qkmw.onrender.com/fms/api/v0/Bins`;
const ShelvesUrl = `https://fms-qkmw.onrender.com/fms/api/v0/Shelvess`;

const BinViewPage = ({ BinId, Bin, goBack }) => {
  const { id } = useParams();
  const effectiveId = BinId || id;
  ...
  const handleUpdate = async () => {
    ...
    const toastId = toast.loading("Updating Bin...");
    try {
      const response = await axios.put(`${mergedUrl}/${effectiveId}`, form);
      if (response.status === 200) {
        toast.update(toastId, {
          render: "Bin updated successfully!",
          type: "success",
          isLoading: false,
          autoClose: 3000,
        });
  ...
  useEffect(() => {
    ...
    const fetchShelves = async () => {
      try {
        ...
        if (res.status === 200) setShelvess(res.data.data || []);
      } catch (err) {
        console.error("Failed to fetch Shelvess", err);
      }
    };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex justify-between items-center">
        <h3 className="text-xl font-semibold">Bin View Page</h3>
      </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <select
                ...
                className="mt-1 w-full p-2 border rounded"
              >
                <option value="">Select a Shelves</option>
                {Shelves.map((w) => (
