...
export default function ColorList({ handleAddProductColor }) {
  ...
  const tabNames = ["All ProductColors", "Active", "Archived"];

  const [activeTab, setActiveTab] = useState(tabNames[0]);
  const [ProductColors, setProductColors] = useState([]);
  const [filteredProductColors, setFilteredProductColors] = useState([]);
  ...
  const fetchProductColors = useCallback(async () => {
    ...
    try {
      ...
      const list = Array.isArray(res.data)
        ? res.data
        : res.data.ProductColors ?? res.data.data ?? [];
      setProductColors(list);
      // Set initial filtered list and summary
      setFilteredProductColors(list);
      setSummary({
        total: list.length,
      ...
      console.error(err);
      setError("Unable to load ProductColors.");
    } finally {
      setLoading(false);
    }
  ...
  const fetchMetrics = useCallback(async () => {
    ...
    try {
      ...
      setSummary({
        total: data.totalProductColors,
        active: data.activeProductColors,
        archived: data.archivedProductColors,
      });
    } catch {
      // ignore
    } finally {
  ...
  useEffect(() => {
    fetchProductColors();
    fetchMetrics();
  }, [fetchProductColors, fetchMetrics]);

  // Apply filters (tab, search, sort)
  useEffect(() => {
    let list = [...ProductColors];
    ...
    setFilteredProductColors(list);
  }, [ProductColors, activeTab, searchTerm, sortOption]);

  // Toggle select all
  const toggleSelectAll = (e) =>
    setSelectedIds(e.target.checked ? filteredProductColors.map((z) => z._id) : []);

  // Delete selected ProductColors
  const handleDeleteSelected = async () => {
    if (!selectedIds.length) {
      toast.info("No ProductColors selected");
      return;
    }
    if (!window.confirm("Delete selected ProductColors?")) return;
    try {
  ...
  if (viewingId)
    return (
      <ProductColorViewPage ProductColorId={viewingId} goBack={() => setViewingId(null)} />
    );
  ...
  return (
    <div>
      ...
      <div className="flex justify-between items-center mb-4">
        <h3 className="text-xl font-semibold">ProductColors</h3>
        <div className="space-x-2">
          <button onClick={handleAddProductColor} className="btn">
            + Add
          </button>
          <button
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        {["total", "active", "archived"].map((key, idx) => (
          <div key={key} className="p-4 bg-gray-50 rounded">
            ...
            <div>{key.charAt(0).toUpperCase() + key.slice(1)} ProductColors</div>
          </div>
        ))}
      </div>
            ...
            <tr>
              <th className="px-4 py-2">
                <input
                  ...
                  checked={
                    selectedIds.length === filteredProductColors.length &&
                    !!filteredProductColors.length
                  }
                />
              </th>
              {columns.map((col) => (
            ...
            {filteredProductColors.length ? (
              filteredProductColors.map((z) => (
                <tr key={z._id} className="hover:bg-gray-100">
                  ...
                  <td className="px-4 py-2">{z.ProductColorAddress || "—"}</td>
