...
const API_BASE = "https://fms-qkmw.onrender.com/fms/api/v0/ProductSizes";
const SITES_API = "https://fms-qkmw.onrender.com/fms/api/v0/sites";

export default function ProductSizeViewPage({ ProductSizeId: propId, goBack }) {
  // Determine ID from prop or URL
  const { id: paramId } = useParams();
  const ProductSizeId = propId || paramId;
  ...
  // Fetch ProductSize detail
  useEffect(() => {
    async function fetchDetail() {
      setLoading(true);
      try {
        const resp = await axios.get(`${API_BASE}/${ProductSizeId}`);
  ...
  const handleUpdate = async () => {
    if (!window.confirm("Are you sure you want to update this ProductSize?"))
      return;
    setLoading(true);
    const toastId = toast.loading("Updating...");
    try {
      const resp = await axios.put(`${API_BASE}/${ProductSizeId}`, formData);
      const updated = resp.data.data || resp.data;
      setProductSize(updated);
      setFormData(updated);
      setIsEditing(false);
  ...
  return (
    <div className="p-6 bg-white rounded shadow">
      ...
      <header className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-semibold">ProductSize Details</h2>
