...
const API_BASE = "https://fms-qkmw.onrender.com/fms/api/v0/ProductColors";
const SITES_API = "https://fms-qkmw.onrender.com/fms/api/v0/sites";

export default function ProductColorViewPage({ ProductColorId: propId, goBack }) {
  // Determine ID from prop or URL
  const { id: paramId } = useParams();
  const ProductColorId = propId || paramId;
  ...
  // Fetch ProductColor detail
  useEffect(() => {
    async function fetchDetail() {
      setLoading(true);
      try {
        const resp = await axios.get(`${API_BASE}/${ProductColorId}`);
  ...
  const handleUpdate = async () => {
    if (!window.confirm("Are you sure you want to update this ProductColor?"))
      return;
    setLoading(true);
    const toastId = toast.loading("Updating...");
    try {
      const resp = await axios.put(`${API_BASE}/${ProductColorId}`, formData);
      const updated = resp.data.data || resp.data;
      setProductColor(updated);
      setFormData(updated);
      setIsEditing(false);
  ...
  return (
    <div className="p-6 bg-white rounded shadow">
      ...
      <header className="flex justify-between items-center mb-6">
        <h2 className="text-2xl font-semibold">ProductColor Details</h2>
