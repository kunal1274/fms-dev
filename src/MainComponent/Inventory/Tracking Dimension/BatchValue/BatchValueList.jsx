...
export default function BatchList({
  ...
  /** ---------- State ---------- */
  const tabNames = ["All Batches", "Active Batches"];
  ...
  const fetchBatches = useCallback(
    async (fromDate = startDate, toDate = endDate) => {
      ...
      try {
        ...
        setBatches(filteredByCreatedAt);
  ...
  const filteredBatches = useMemo(() => {
    ...
    return list;
  }, [batches, activeTab, statusFilter, searchTerm, sortOption]);
  ...
  const toggleSelectAll = (e) => {
    setSelectedIds(
      e.target.checked ? filteredBatches.map((c) => getId(c)) : []
    );
  };
  ...
  const handleDeleteSelected = async () => {
    if (!selectedIds.length) {
      toast.info("No batch selected to delete");
      return;
    }
    if (
      !window.confirm(
        `Delete ${selectedIds.length} selected batch${
  ...
  /** ---------- Export ---------- */
  const exportToExcel = () => {
    const rows = filteredBatches.length ? filteredBatches : batches;
    if (!rows.length) {
  ...
  const generatePDF = () => {
    ...
    doc.save("batch_list.pdf");
  };

  /** ---------- View toggle ---------- */
  const goBack = () => setViewingBatchId(null);
  ...
  if (viewingBatchId) {
    return (
      <div className="p-4">
        <BatchViewPagee batchId={viewingBatchId} goBack={goBack} />
      </div>
    );
  }
          ...
          <div className="space-y-6">
            ...
            <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex items-center space-x-2 ">
                <h3 className="text-xl font-semibold">Batch List</h3>
              </div>
            ...
            <div className=" bg-white rounded-lg ">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
                {[
                  ["Total Batches", summary.count],
                  ["Active Batches", summary.activeBatches],
                  ["Archived Batches", summary.archivedBatches],
                  [
                    "Inactive (calc)",
                    Math.max(summary.count - summary.activeBatches, 0),
                  ],
                ].map(([label, value]) => (
                  <div
                    key={label}
              ...
              <div className="flex flex-wrap items-center gap-3 md:gap-4">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <option value="name-asc">Batch Name</option>
                    <option value="code-asc">Batch Code in Ascending</option>
                    <option value="code-desc">Batch Code in Descending</option>
                  </select>
                </div>
                      ...
                      >
                        ...
                        <td className="px-6 py-4">
                          <button
                            ...
                            onClick={() => handleRowClick(getId(c))}
                          >
                            {getCode(c)}
                          </button>
                        </td>
