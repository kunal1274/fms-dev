...
const API_BASE = "https://fms-qkmw.onrender.com/fms/api/v0/Products";
const SITES_API = "https://fms-qkmw.onrender.com/fms/api/v0/sites";

export default function ProductViewPage({ ProductId: propId, goBack }) {
  ...
  useEffect(() => {
    async function fetchDetail() {
      setLoading(true);
      try {
        const resp = await axios.get(`${API_BASE}/${ProductId}`);
  ...
  const handleUpdate = async () => {
    ...
    try {
      const resp = await axios.put(`${API_BASE}/${ProductId}`, formData);
