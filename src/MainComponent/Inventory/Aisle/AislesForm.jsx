...
export default function Ailses({ handleCancel }) {
  ...
  const apiBase = "https://fms-qkmw.onrender.com/fms/api/v0/Ailsess";
  ...
  const createAilses = async (e) => {
    ...
    try {
      ...
      payload.append("AilsesAddress", form.AilsesAddress);
      ...
      toast.success("Ailses created successfully");
      handleCancel();
    } catch (err) {
      console.error("Create error:", err.response || err);
      const msg = err.response?.data?.message || "Error creating Ailses";
      toast.error(msg);
    }
  };
  ...
  return (
    <div className="p-4">
      <ToastContainer />
      <h3 className="text-xl font-semibold mb-4">Ailses Form</h3>
      <form
        onSubmit={createAilses}
        ...
        <section className="p-6 grid grid-cols-1 sm:grid-cols-2 gap-6">
          ...
          <div>
            ...
            <input
              ...
              value={form.name}
              onChange={handleChange}
              placeholder="e.g. Central Ailses"
              required
              className="mt-1 w-full p-2 border rounded focus:ring-2 focus:ring-blue-200"
            />
          </div>
