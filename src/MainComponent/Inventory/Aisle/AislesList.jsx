...
export default function Ailses({ handleAddProduct }) {
  ...
  const fetchMetrics = useCallback(async () => {
    ...
    try {
      ...
      setSummary({
        ...
        archived: data.archivedProducts,
      });
    } catch {
      // ignore
    } finally {
  ...
  const toggleSelectAll = (e) =>
    setSelectedIds(e.target.checked ? filteredProducts.map((z) => z._id) : []);
  ...
  if (viewingId)
    return (
      <ProductViewPage ProductId={viewingId} goBack={() => setViewingId(null)} />
    );
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        {["total", "active", "archived"].map((key, idx) => (
          <div key={key} className="p-4 bg-gray-50 rounded">
            ...
            <div>{key.charAt(0).toUpperCase() + key.slice(1)} Products</div>
          </div>
        ))}
      </div>
            ...
            {filteredProducts.length ? (
              filteredProducts.map((z) => (
                <tr key={z._id} className="hover:bg-gray-100">
                  ...
                  <td className="px-4 py-2">{z.ProductAddress || "—"}</td>
