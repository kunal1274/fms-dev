...
const RacksForm = ({ handleCancel }) => {
  const [form, setForm] = useState({});
  const apiBase = "https://fms-qkmw.onrender.com/fms/api/v0/Racks";

  // ─── Data ────────────────────────────────────────────────
  const [Rackss, setRacks] = useState([]);

  // ─── Helpers ─────────────────────────────────────────────

  // ─── Load existing Rackss once ────────────────────────
  const handleChange = () => {};
  const createRacks = async (e) => {
    ...
    try {
      ...
      const newRacks = data.data;

      toast.success("Racks saved", {
        autoClose: 1200,
        onClose: () => handleCancel(),
      });

      setRackss((prev) => [...prev, newRacks]);

      onSaved?.(newRacks);
    } catch (err) {
      console.error("Error creating Racks:", err.response || err);
      // const msg = err.response?.data?.message || "Couldn’t save Racks"; // ← define msg properly
  ...
  const handleReset = () => {
    const newRacksCode = generateAccountNo(Rackss);
    setForm({ ...initialForm, RacksAccountNo: newRacksCode });
  };
  const handleEdit = () => {
    navigate("/Racksview", { state: { Racks: formData } });
  };
    ...
    <div className="">
      ...
      <div className="flex justify-between ">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-xl font-semibold">Racks Form</h3>
        </div>
      </div>
