...
export default function RackForm({ handleCancel }) {
  const [form, setForm] = useState({});
  const apiBase = "https://fms-qkmw.onrender.com/fms/api/v0/Rack";
  ...
  const createRack = async (e) => {
    ...
    try {
      ...
      console.error("Error creating Rack:", err.response || err);
      // const msg = err.response?.data?.message || "Couldn’t save Rack"; // ← define msg properly
  ...
  const handleReset = () => {
    const newRackCode = generateAccountNo(Racks);
    setForm({ ...initialForm, RackAccountNo: newRackCode });
  };
  const handleEdit = () => {
    navigate("/Rackview", { state: { Rack: formData } });
  };
    ...
    <div className="">
      ...
      <div className="flex justify-between ">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-xl font-semibold">Rack Form</h3>
        </div>
      </div>
