...
export default function ProductStyleList({ handleAddProductStyle }) {
  ...
  const [filteredProductStyles, setFilteredProductStyles] = useState([]);
  ...
  // Fetch ProductStyles list
  const fetchProductStyles = useCallback(async () => {
    setLoading(true);
    setError(null);
    try {
      ...
      const list = Array.isArray(res.data)
        ? res.data
        : res.data.ProductStyles ?? res.data.data ?? [];
      setProductStyles(list);
      // Set initial filtered list and summary
      setFilteredProductStyles(list);
      setSummary({
        total: list.length,
      ...
      console.error(err);
      setError("Unable to load ProductStyles.");
    } finally {
      setLoading(false);
    }
  ...
  const fetchMetrics = useCallback(async () => {
    ...
    try {
      ...
      setSummary({
        total: data.totalProductStyles,
        active: data.activeProductStyles,
        archived: data.archivedProductStyles,
      });
    } catch {
      // ignore
    } finally {
  ...
  useEffect(() => {
    fetchProductStyles();
    fetchMetrics();
  }, [fetchProductStyles, fetchMetrics]);

  // Apply filters (tab, search, sort)
  useEffect(() => {
    let list = [...ProductStyles];
    ...
    setFilteredProductStyles(list);
  }, [ProductStyles, activeTab, searchTerm, sortOption]);
  ...
  const handleDeleteSelected = async () => {
    if (!selectedIds.length) {
      toast.info("No ProductStyles selected");
      return;
    }
    if (!window.confirm("Delete selected ProductStyles?")) return;
    try {
            ...
            <tr>
              <th className="px-4 py-2">
                <input
                  ...
                  checked={
                    selectedIds.length === filteredProductStyles.length &&
                    !!filteredProductStyles.length
                  }
                />
              </th>
              {columns.map((col) => (
            ...
            {filteredProductStyles.length ? (
              filteredProductStyles.map((z) => (
                <tr key={z._id} className="hover:bg-gray-100">
                  ...
                  <td className="px-4 py-2">{z.ProductStyleAddress || "—"}</td>
