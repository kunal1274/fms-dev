...
const ColorViewPage = ({ ColorId, goBack, toggleView }) => {
  ...
  useEffect(() => {
    const fetchColor = async () => {
      try {
        const res = await axios.get(`${baseUrl}/${colorId}`);
        if (res.status === 200) {
        ...
        const errorMsg =
          err.response?.data?.message || "Failed to fetch color data.";
        setError(errorMsg);
      } finally {
        setLoading(false);
      }
  ...
  const handleUpdate = async () => {
    ...
    try {
      ...
      const res = await axios.put(`${baseUrl}/${colorId}`, payload);
      if (res.status === 200) {
  ...
  if (loading) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-gray-100">
        ...
        <p className="mt-4 text-gray-600 text-lg">Loading color data...</p>
      </div>
    );
  }
  ...
  return (
    <div className="p-6 bg-white">
      ...
      <div className="flex justify-between items-center mb-6">
        <h2 className="text-xl font-semibold">Color View Page</h2>
