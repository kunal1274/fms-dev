...
export default function ItemList({ handleAddItem }) {
    ...
    async ({ fromDate, toDate } = {}) => {
      ...
      try {
        ...
        setSummary((prev) => ({
          ...
          paidItems: finalList.filter((c) => getStatus(c) === "Paid").length,
          activeItems: finalList.filter(isActive).length,
          onHoldItems: finalList.filter((c) => isInactive(c) || isOnHold(c))
            .length,
        }));
      } catch (err) {
        console.error(err);
    ...
    async ({ fromDate, toDate } = {}) => {
      ...
      try {
        ...
        setSummary((prev) => ({
          ...
          onHoldItems:
            typeof m?.inactiveItems === "number"
              ? m.inactiveItems
              : m?.onHoldItems ?? prev.onHoldItems,
        }));
      } catch (err) {
  ...
  const handleSortChange = (e) => {
    ...
    if (v === "Item Account in Ascending") return setSortOption("code-asc");
    if (v === "Item Account in descending") return setSortOption("code-desc");
    setSortOption(v || "");
  };
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      if (succeeded) {
        ...
        setSelectedIds([]);
        if (isRangeValid) {
          await fetchItems({ fromDate: startDate, toDate: endDate });
        ...
        <div className="flex gap-2">
          ...
          <button
            onClick={() => {
              ...
              fetchItems({ fromDate: startDate, toDate: endDate });
            }}
        ...
        <div className="flex items-center space-x-4">
          ...
          <div className="relative">
            ...
            >
              ...
              <option value="Item Account in Ascending">
                Item Account in Ascending
              </option>
              <option value="Item Account in descending">
                Item Account in descending
              </option>
            </select>
          </div>
