...
const JournalForm = ({ handleCancel }) => {
  const [form, setForm] = useState({});
  const apiBase = "https://fms-qkmw.onrender.com/fms/api/v0/Journal";

  // ─── Data ────────────────────────────────────────────────
  const [Journals, setJournals] = useState([]);
  useEffect(() => {
    const apiJournalurl = async () => {
      try {
  ...
  const createJournal = async (e) => {
    ...
    try {
      ...
      console.error("Error creating Journal:", err.response || err);
      // const msg = err.response?.data?.message || "Couldn’t save Journal"; // ← define msg properly
  ...
  const handleReset = () => {
    const newJournalCode = generateAccountNo(Journals);
    setForm({ ...initialForm, JournalAccountNo: newJournalCode });
  };
  const handleEdit = () => {
    navigate("/Journalview", { state: { Journal: formData } });
  };
  return (
    <div className="">
      <ToastContainer />
      ...
      <div className="flex justify-between ">
        <div className="flex items-center space-x-2">
          ...
          <h3 className="text-xl font-semibold">Journal Form</h3>
        </div>
      </div>
