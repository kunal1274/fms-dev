...
export default function CreditNoteList({ handleAddCreditNote }) {
  /** ---------- API ---------- */
  ...
  const [customers, setCreditNotes] = useState([]);
  const [selectedIds, setSelectedIds] = useState([]);
  const [viewingCreditNoteId, setViewingCreditNoteId] = useState(null);
    ...
    async ({ fromDate, toDate } = {}) => {
      ...
      try {
        ...
        setSummary((prev) => ({
          ...
          paidCreditNotes: finalList.filter((c) => getStatus(c) === "Paid")
            .length,
          activeCreditNotes: finalList.filter(isActive).length,
          onHoldCreditNotes: finalList.filter(
            (c) => isInactive(c) || isOnHold(c)
          ).length,
        }));
      } catch (err) {
        console.error(err);
        setError("Unable to load CreditNote data.");
      } finally {
        setLoading(false);
      }
    },
  ...
  const fetchMetrics = useCallback(
    async ({ fromDate, toDate } = {}) => {
      ...
      try {
        ...
        setSummary((prev) => ({
          ...prev,
          count: m?.totalCreditNotes ?? prev.count,
          creditLimit: m?.creditLimit ?? prev.creditLimit,
          paidCreditNotes: m?.paidCreditNotes ?? prev.paidCreditNotes,
          activeCreditNotes: m?.activeCreditNotes ?? prev.activeCreditNotes,
          onHoldCreditNotes:
            typeof m?.inactiveCreditNotes === "number"
              ? m.inactiveCreditNotes
              : m?.onHoldCreditNotes ?? prev.onHoldCreditNotes,
        }));
      } catch (err) {
  ...
  useEffect(() => {
    fetchCreditNotes(); // all
    fetchMetrics(); // all
  }, [fetchCreditNotes, fetchMetrics]);
  ...
  const handleSortChange = (e) => {
    const v = e.target.value;
    if (v === "CreditNote Name") return setSortOption("name-asc");
    if (v === "CreditNote Account in Ascending")
      return setSortOption("code-asc");
    if (v === "CreditNote Account in descending")
  ...
  const toggleSelectAll = (e) => {
    setSelectedIds(e.target.checked ? filteredCreditNotes.map(getId) : []);
  };
  ...
  const handleDeleteSelected = async () => {
    ...
    try {
      ...
      if (succeeded) {
        ...
        setSelectedIds([]);
        if (isRangeValid) {
          await fetchCreditNotes({ fromDate: startDate, toDate: endDate });
  ...
  const generatePDF = () => {
    ...
    autoTable(doc, {
      ...
      body: filteredCreditNotes.map((c, i) => [
        i + 1,
        getCode(c) || "",
        getName(c) || "",
        getEmail(c) || "",
  ...
  const handleCreditNoteClick = (customerId) => {
    setViewingCreditNoteId(customerId);
  };

  const goBack = () => setViewingCreditNoteId(null);

  /** ---------- Render ---------- */
  if (loading) return <div>Loading…</div>;
  if (error) return <div className="text-red-600">{error}</div>;

  if (viewingCreditNoteId) {
    return (
      <div className="p-4">
        <CreditNoteViewPage customerId={viewingCreditNoteId} goBack={goBack} />
      </div>
    );
  }
    ...
    <div>
      ...
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="flex items-center space-x-2 ">
          <h3 className="text-xl font-semibold mb-6">CreditNote List</h3>
        </div>
