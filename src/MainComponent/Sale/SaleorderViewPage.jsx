  ...
  <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
    <div className="bg-white p-6 rounded-2xl shadow-2xl w-96">
      <h2 className="text-xl font-semibold mb-4">
        Confirm Change to “{status}”
      </h2>
      <div className="space-y-2 text-sm">
        <p>
          <strong>Sale Order #:</strong> {saleData.orderNum}
        </p>
        <p>
        ...
        <p>
          ...
          {selectedItem?.name || saleData.item?.name}
        </p>
        <p>
          <strong>Confirm Qty:</strong> {quantity}
        </p>
      </div>
...
const SaleOrderViewPage = ({ goBack, saleId }) => {
  ...
  // wrap your existing handleStatusUpdate so it closes the modal
  const confirmStatusChange = async () => {
    await handleStatusUpdate(statusToChange);
    closeStatusModal();
  };
  // Component states
