...
const CustomerViewPage = ({
  ...
  const handleChange = (e) => {
    ...
    if (name === "bankType") {
      const noBank = ["Cash", "Barter", "Crypto", "UPI"].includes(val.trim()); //   1
      setFormData((prev) => ({
        //   2
        ...prev,
        bankType: val.trim(),
        bankDetails: prev.bankDetails.map(
          (
            b //   3
          ) =>
            noBank
    ...
    if (name === "qrDetails" && (!/^[A-Z0-9.@]*$/.test(val) || val.length > 25))
      return;
    if (name === "Tannumber" && !/^[A-Z0-9]{0,25}$/.test(val)) return;
    if (name === "panNum" && !/^[A-Z0-9]{0,10}$/.test(val)) return;
    if (name === "registrationNum" && !/^[A-Z0-9]{0,15}$/.test(val)) return;
    if (typeof val === "string") val = val.trim();

    // If user picks a "no-bank-details" type, clear all nested bankDetails
    if (name === "bankType") {
      const isNoBank = ["Cash", "Barter", "Crypto", "UPI"].includes(val);
      setFormData((prev) => ({
        ...prev,
        bankType: val,
        // either wipe the array entirely:
        bankDetails: isNoBank ? [] : prev.bankDetails,
        // or zero out each field if you must keep the same number of rows:
    ...
    <div className="">
      ...
      <form className="bg-white shadow-none rounded-lg divide-y divide-gray-200">
        ...
        {/* <section className="p-6">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-6">
            {formData.bankDetails?.map((b, i) => {
              const noBank = ["Cash", "Barter"].includes(b.type);
                ...
                <div key={i} className="space-y-4">
                  ...
                  <div>
                    ...
                    <select
                      value={b.type}
                      onChange={(e) =>
                        handleBankDetailChange(i, "type", e.target.value)
                      }
                      disabled={!isEditing}
                      className="mt-1 w-full p-2 border rounded focus:ring-2 focus:ring-blue-200"
                    >
