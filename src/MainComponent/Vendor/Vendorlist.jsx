...
export default function VendorList({ handleAddVendor, onView }) {
  ...
  const fetchVendors = useCallback(
    async (fromDate = startDate, toDate = endDate) => {
      ...
      try {
        ...
        +setFilteredVendors(list); // ← Add this line to update the visible Vendors immediately

        setVendorSummary({
          count: list.length,
          creditLimit: list.reduce((s, c) => s + (c.creditLimit || 0), 0),
          paidVendors: list.filter((c) => c.status === "Paid").length,
          activeVendors: list.filter((c) => c.active).length,
          onHoldVendors: list.filter((c) => c.onHold).length,
        });
      } catch (err) {
        console.error(err);
  ...
  }, [vendorList, activeTab, statusFilter, searchTerm, sortOption]);
  ...
  const toggleSelectAll = (e) => {
    setSelectedVendors(
      e.target.checked ? filteredVendors.map((c) => c._id) : []
    );
  };
  ...
  const handleDeleteSelected = async () => {
    if (!selectedVendors.length) {
      toast.info("No Vendors selected to delete");
      return;
    }
    ...
    try {
      const results = await Promise.allSettled(
        selectedVendors.map((id) => axios.delete(`${baseUrl}/${id}`))
      );
            ...
            <div className="space-y-6">
              ...
              <div className=" bg-white rounded-lg ">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-5 gap-4">
                  {[
                    ["Total Vendors", vendorSummary.count],
                    ["Credit Limit", vendorSummary.creditLimit],
                    ["Paid Vendors", vendorSummary.paidVendors],
                    ["Active Vendors", vendorSummary.activeVendors],
                    ["On‑Hold Vendors", vendorSummary.onHoldVendors],
                  ].map(([label, value]) => (
                    <div
                      key={label}
                        ...
                        >
                          ...
                          >
                            <button
                              ...
                              onClick={() => handleVendorClick(c._id)}
                            >
                              {c.code}
                            </button>
                          </td>
