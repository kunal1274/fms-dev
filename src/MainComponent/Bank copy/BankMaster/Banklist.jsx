...
export default function BankList({ handleAddBank, onView }) {
  const baseUrl = "https://fms-qkmw.onrender.com/fms/api/v0/Banks";
  ...
  const fetchBanks = useCallback(
    async (fromDate = startDate, toDate = endDate) => {
      ...
      try {
        ...
        setBankList(list);
        setFilteredBanks(list); // <-- fixed (removed stray '+')

        setBankSummary({
          count: list.length,
          creditLimit: list.reduce((s, c) => s + (c.creditLimit || 0), 0),
          paidBanks: list.filter((c) => c.status === "Paid").length,
          activeBanks: list.filter((c) => c.active).length,
          onHoldBanks: list.filter((c) => c.onHold).length,
        });
      } catch (err) {
        console.error(err);
  ...
  const fetchMetrics = useCallback(async () => {
    try {
      ...
      setBankSummary((prev) => ({
        ...prev,
        count: m.totalBanks ?? prev.count,
        creditLimit: m.creditLimit ?? prev.creditLimit,
        paidBanks: m.paidBanks ?? prev.paidBanks,
        activeBanks: m.activeBanks ?? prev.activeBanks,
        onHoldBanks: m.onHoldBanks ?? prev.onHoldBanks,
      }));
    } catch (err) {
      console.error(err);
    } finally {
  ...
  }, [BankList, activeTab, statusFilter, searchTerm, sortOption, tabNames]);
  ...
  const toggleSelectAll = (e) => {
    setSelectedBanks(e.target.checked ? filteredBanks.map((c) => c._id) : []);
  };
  ...
  const handleDeleteSelected = async () => {
    if (!selectedBanks.length) {
      toast.info("No Banks selected to delete");
      return;
    }
    if (!window.confirm("Delete selected Banks?")) return;

    try {
      const results = await Promise.allSettled(
        selectedBanks.map((id) => axios.delete(`${baseUrl}/${id}`))
      );
  ...
  const exportToExcel = () => {
    ...
    const ws = XLSX.utils.json_to_sheet(BankList);
  ...
  const generatePDF = () => {
    ...
    autoTable(doc, {
      ...
      body: filteredBanks.map((c, i) => [i + 1]),
    });
    doc.save("Bank_list.pdf");
  };

  const handleBankClick = (BankId) => setViewingBankId(BankId);
  const resetFilters = () => {
    setSearchTerm("");
    setStatusFilter("All");
    setSortOption("");
  };
  const goBack = () => setViewingBankId(null);
  ...
  if (viewingBankId) {
    return (
      <div className="p-4">
        <BankViewPage BankId={viewingBankId} goBack={goBack} />
      </div>
    );
  }
              ...
              <div className="flex items-center space-x-4">
                ...
                <div className="relative">
                  ...
                  >
                    ...
                    <option value="code-asc">Bank Account in Ascending</option>
                    <option value="code-desc">
                      Bank Account in descending
                    </option>
                  </select>
                </div>
