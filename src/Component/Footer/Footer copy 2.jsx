...
const Footer = () => {
  ...
  // Tick every second (local machine time)
  useEffect(() => {
  ...
  // Use the user's browser locale + timezone, with a short TZ label
  const userLocale =
  ...
  const timeFormatter = useMemo(
    () =>
      new Intl.DateTimeFormat(userLocale, {
        ...
        minute: "2-digit",
        second: "2-digit",
        timeZoneName: "short", // shows IST/PDT/etc.
      }),
    [userLocale]
  );
    ...
    <footer className="bg-white border-b h-12 border-gray-200 px-3 py-2 sticky top-0 z-10 shadow-sm">
      <div className="flex items-center justify-end h-full text-gray-600">
        <div className="flex items-center gap-4">
          ...
          {/* Clock Display (visitor's local time) */}
